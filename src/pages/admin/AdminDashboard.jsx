...
import { ShoppingBag, Users, Store, Truck, TrendingUp } from 'lucide-react'
  ...
  return (
    <div className="page">
      ...
      <div className="stat-grid" style={{ marginBottom: 28 }}>
        {[
          { label: "Today's revenue", value: formatCurrency(stats.revenue), icon: TrendingUp, color: 'var(--poa-green-dark)' },
          { label: 'Total orders', value: stats.orders, icon: ShoppingBag },
          { label: 'Pending orders', value: stats.pending, icon: ShoppingBag, color: '#D97706' },
          ...
          <div key={label} className="stat-card">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                <div className="stat-value" style={color ? { color, fontSize: 22 } : { fontSize: 28 }}>{value}</div>
        ...
        : (
          <div className="table-wrap">
            <table>
              <thead>
                <tr>
                  <th>Order</th><th>Status</th><th>Amount</th><th>Date</th>
                </tr>
              </thead>
              <tbody>
                {recentOrders.map(o => (
                  <tr key={o.id}>
                    <td style={{ fontWeight: 600, fontSize: 13 }}>{o.order_number || o.id.slice(0,8)}</td>
                    ...
                    <td style={{ color: 'var(--text-muted)', fontSize: 12 }}>{formatDateTime(o.created_at)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
