  ...
  return (
    <div className="page">
      ...
      <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 20, flexWrap: 'wrap' }}>
        ...
        {selectedVendor && (
          <button onClick={toggleOpen} style={{ padding: '8px 14px', borderRadius: 99, border: 'none', cursor: 'pointer', fontWeight: 700, fontSize: 12, background: selectedVendor.is_open ? 'var(--poa-green-light)' : '#FEE2E2', color: selectedVendor.is_open ? 'var(--poa-green-dark)' : '#DC2626', whiteSpace: 'nowrap' }}>
      ...
      <div style={{ marginBottom: 16 }}>
        <h1 className="page-title">{selectedId === 'all' ? 'All Restaurants' : selectedVendor?.name || '...'}</h1>
        <p style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 2 }}>
          {selectedId === 'all' ? `${vendors.length} restaurants` : selectedVendor?.address}
        </p>
      </div>

      {/* Stats */}
      <div className="stat-grid" style={{ marginBottom: 20 }}>
        <div className="stat-card"><div className="stat-value" style={{ color: '#D97706' }}>{stats.pending}</div><div className="stat-label">Pending</div></div>
        <div className="stat-card"><div className="stat-value">{stats.active}</div><div className="stat-label">Active orders</div></div>
        <div className="stat-card"><div className="stat-value" style={{ fontSize: 18, color: 'var(--poa-green-dark)' }}>{formatCurrency(stats.todayRevenue)}</div><div className="stat-label">Today's revenue</div></div>
        <div className="stat-card"><div className="stat-value">{stats.todayOrders}</div><div className="stat-label">Delivered today</div></div>
      </div>

      {/* Status filter tabs */}
      <div style={{ display: 'flex', gap: 6, marginBottom: 16 }}>
        {[['active', 'Active only'], ['all', 'All orders']].map(([val, label]) => (
          <button key={val} onClick={() => setStatusFilter(val)} style={{
            padding: '6px 14px', borderRadius: 99, border: 'none', cursor: 'pointer',
            fontSize: 13, fontWeight: 600,
            background: statusFilter === val ? 'var(--poa-green)' : 'var(--bg-elevated)',
      ...
      {loading ? (
        ...
        <div className="card" style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>
          ...
          <p style={{ fontSize: 13, marginTop: 4 }}>
            {statusFilter === 'active' ? 'No active orders right now' : 'No orders yet for this selection'}
          </p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {orders.map(order => (
            <div key={order.id} className="card fade-in" style={{ borderLeft: `4px solid ${order.status === 'pending' ? '#F59E0B' : order.status === 'ready' ? 'var(--poa-green)' : 'var(--border)'}` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                <div>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 2 }}>
                    ...
                    {selectedId === 'all' && order.vendors && (
                      <span style={{ marginLeft: 6, background: 'var(--bg-elevated)', padding: '1px 6px', borderRadius: 4, fontSize: 11 }}>
                        {order.vendors.name}
                      </span>
                    )}
                  </div>
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  <span className={`badge ${STATUS_COLOR[order.status] || 'badge-gray'}`} style={{ textTransform: 'capitalize' }}>
                    {order.status.replace(/_/g,' ')}
                  </span>
                  <span style={{ fontWeight: 700, color: 'var(--poa-green-dark)' }}>{formatCurrency(order.total_amount)}</span>
                </div>
              </div>
              ...
              <div style={{ marginBottom: 8 }}>
                {order.order_items?.map((item, i) => (
                  <div key={i} style={{ fontSize: 13, color: 'var(--text-secondary)', padding: '1px 0' }}>
              ...
              {order.delivery_notes && (
                <div style={{ fontSize: 12, color: 'var(--poa-orange)', marginBottom: 8, background: 'var(--poa-orange-light)', padding: '6px 10px', borderRadius: 6 }}>
                  📝 {order.delivery_notes}
                </div>
              )}
              ...
              {/* Action */}
              {order.status === 'ready' ? (
                <span style={{ fontSize: 13, color: 'var(--poa-green-dark)', fontWeight: 600 }}>✅ Waiting for rider</span>
              ) : NEXT[order.status] && (
                <button className="btn btn-primary btn-sm" onClick={() => advance(order.id, NEXT[order.status].next, order)}>
